...
class DNS_Request:
    ...
    def __init__(self, raw_data):
        ...
        self._id = DNSRecord.parse(raw_data).header.id
    ...
    def generate_response(self, request, response):
        ...
        """
        if response[0] == "CNAME":
            return DNS_Response(DNSRecord(header=DNSHeader(id=request.id, qr=1, aa=1, ra=1, rcode=DNS_Rcode.NoError.value),
    ...
    @classmethod
    def generate_error_response(cls, request, error_code):
        """
        This function generates a dns response with specific error code.
        return: DNS_Response object
        usage: req_obj.generate_error_response(DNS_Rcode.FormErr)
        """
        return DNS_Response(DNSRecord(header=DNSHeader(id=request.id, qr=1, aa=1, ra=1, rcode=error_code.value)).pack())

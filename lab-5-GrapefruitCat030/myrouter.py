...
class Router(object):
    ...
    def icmp_errorhandle(self, packet, type, childtype):
        ...
        if (icmph is not None) and ((icmph.icmptype == ICMPType.DestinationUnreachable) or (icmph.icmptype == ICMPType.TimeExceeded)) :
            print(f"GOOD BOY, type : {type} ,current packet : {packet}")
            return None;
    ...
    def handle_packet(self, recv: switchyard.llnetbase.ReceivedPacket):
        ...
        if arp is not None:                        #  prepare for Arp processing.
            print("____The packet is ARP.____")
            custom_assert(ip4 is None)
            dstipaddr = arp.targetprotoaddr
            flag = State.isarp
        elif icmph is not None:                     #  prepare for ICMP processing.                          
            print("____The packet is ICMP.____")
            custom_assert(ip4 is not None)
            assert(isinstance(icmph, ICMP))
            flag = State.iscommon
            dstipaddr = ip4.dst
            # Prior to a forwarding decision (i.e., a forwarding table lookup), 
            # you should first check whether the IP destination address is 
            # among one of the router's interfaces. 
            if icmph.icmptype == ICMPType.EchoRequest:
                for intf in self.my_interfaces:
                    if intf.ipaddr == dstipaddr:
                        print("____The packet is ICMP to router.____")
                        flag = State.isicmp
                        dstipaddr = ip4.src
        else:                                      #  prepare for COMMON processing.
            print("____The packet is COMMON.____")
            custom_assert(ip4 is not None)
            flag = State.iscommon
            dstipaddr = ip4.dst
        ...
        # If a COMMON packet is for the router itself (i.e., destination address is among the router's interfaces), just drop/ignore the packet.
        if flag is State.iscommon:
            for intf in self.my_interfaces:
                if intf.ipaddr == dstipaddr:
                    if (icmph is not None) and (icmph.icmptype != ICMPType.EchoReply):
                        return None
                    self.icmp_errorhandle(packet, ICMPType.DestinationUnreachable, 3)
                    print("drop it! drop it!")
                    return None


        # variaty of packet processing.
        ...
        if flag == State.isarp:
            ...
            # If there is no match in the table, drop the packet for now, and send ICMP error.
            destentry = self.router_table.LPM(dstipaddr)
            if destentry == None:
                print("COMMON. Cannot find destentry.")
                self.icmp_errorhandle(packet, ICMPType.DestinationUnreachable, 0)
                return None
            # If TTL expired, send ICMP error.
            if (packet[IPv4].ttl - 1) <= 0:
                print("COMMON. TTL is expired.")
                self.icmp_errorhandle(packet, ICMPType.TimeExceeded, 0)
                return None
    ...
    def queueoperate(self):
        ...
        for dealip, item in list(self.arp_resolved_queue.items()):
            ...
            if temprc is retranState.SEND_ARPOK:
                ...
                print(f"DELETE : delete the item for retransmit.{item}\n")
                for pkt in item.pkt_list:
                    self.icmp_errorhandle(pkt, ICMPType.DestinationUnreachable, 1)
                del self.arp_resolved_queue[dealip]
    ...
    def start(self):
       ...
       #custom_assert(0, "it should be 1")
        ...
        '''
        log_info(f"****That is my interface ip! {[i.ipaddr  for i in self.net.interfaces()]}")
        log_info(f"****That is my interface mac!{[i.ethaddr for i in self.net.interfaces()]}")
        self.printtable()
